...
class IssueBloc extends Bloc<IssueEvent, IssueState> {
  ...
  IssueBloc({
    ...
    on<FetchIssueEvent>((event, emit) async {
      ...
      result.fold((failure) => emit(IssueFailure(failure.message)), (posts) {
    ...
    on<AddACommentEvent>((event, emit) async {
      ...
      result.fold((failure) => emit(IssueLoaded(previousPosts)), (_) {
    ...
    on<AddAgreeVoteEvent>((event, emit) async {
      ...
      result.fold((failure) => emit(IssueLoaded(previousPosts)), (_) {
        print("Agree Or DisAgree Vote on Authority FeedBacks-> successfully!");
        final updatedPosts = previousPosts.map((post) {
          if (post.id == event.post.id) {
            //update this post directly from event.post
            post = event.post;
    ...
    on<DeletePostEvent>((event, emit) async {
      ...
      result.fold((failure) => emit(IssueLoaded(previousPosts)), (_) {
        ...
        final updatedPosts = previousPosts
